...
function GoogleReviewsContent() {
  ...
  const goToPrevious = () => {
    setCurrentReviewIndex((prevIndex) => {
      ...
      return newIndex < 0 ? Math.max(0, highRatedReviews.length - 2) : newIndex;
    });
  };
  ...
  const goToNext = () => {
    setCurrentReviewIndex((prevIndex) => {
      const newIndex = prevIndex + 2;
      return newIndex >= highRatedReviews.length ? 0 : newIndex;
    });
  };
  ...
  if (state.reviews.length === 0 && !state.error) {
    return (
      <section className="py-12 md:py-20 bg-[#F0F5FF]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-center text-[#dc2626] mb-10 md:mb-16">
  ...
  return (
    <section className="py-12 md:py-20 bg-[#F0F5FF]" suppressHydrationWarning={true}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-center text-[#dc2626] mb-8 md:mb-16">
          What Our Customers Say
        </h2>
        
        <div className="relative overflow-x-hidden overflow-y-hidden w-full">
          <div className="flex items-stretch justify-center gap-4 md:gap-6 lg:gap-8 overflow-x-auto overflow-y-hidden pb-2 snap-x snap-mandatory [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {/* Show two reviews at a time + Review us card */}
            {highRatedReviews.slice(currentReviewIndex, currentReviewIndex + 2).map((review, index) => (
              <div key={index} className="bg-white p-5 sm:p-6 md:p-8 rounded-xl shadow-lg w-[280px] min-w-[280px] sm:w-72 sm:min-w-[18rem] md:w-80 md:min-w-[20rem] lg:w-96 lg:min-w-[24rem] min-h-[320px] md:min-h-[384px] flex flex-col flex-shrink-0 snap-center">
                ...
                <p className="text-gray-700 mb-4 flex-grow text-sm md:text-base line-clamp-5 overflow-hidden">
                  {review.text}
                </p>
                <div className="mt-auto flex items-center pt-2">
                  <div className="h-9 w-9 sm:h-10 sm:w-10 rounded-full bg-[#1e3a8a] flex items-center justify-center mr-2 sm:mr-3 flex-shrink-0">
                  ...
                  <div className="min-w-0">
                    <p className="font-semibold text-[#1e3a8a] text-sm sm:text-base truncate">{review.author_name}</p>
                    <p className="text-xs sm:text-sm text-gray-500">{review.relative_time}</p>
                  </div>
                </div>
              </div>
            ))}
            ...
            <div className="bg-white p-5 sm:p-6 md:p-8 rounded-xl shadow-lg w-[280px] min-w-[280px] sm:w-72 sm:min-w-[18rem] md:w-80 md:min-w-[20rem] lg:w-96 lg:min-w-[24rem] min-h-[320px] md:min-h-[384px] flex flex-col flex-shrink-0 snap-center">
          ...
          {/* Navigation Arrows */}
          {highRatedReviews.length > 2 && (
            <>
              <button 
                onClick={goToPrevious}
                className="absolute left-0 top-1/2 -translate-y-1/2 bg-gray-300 p-3 rounded-full shadow-md hover:bg-gray-400 transition-colors z-10"
              >
              ...
              <button 
                onClick={goToNext}
                className="absolute right-0 top-1/2 -translate-y-1/2 bg-gray-300 p-3 rounded-full shadow-md hover:bg-gray-400 transition-colors z-10"
              >
          ...
          <div className="flex justify-center mt-6 md:mt-8 space-x-2">
            {Array.from({ length: Math.ceil(highRatedReviews.length / 2) }).map((_, index) => (
              <button
                key={index}
